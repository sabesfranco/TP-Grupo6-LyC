...
public class AsmCodeGenerator implements FileGenerator {
    ...
    public void generate(FileWriter fileWriter) throws IOException {
        ...
        // Generate code from intermediate code
        ...
        for (SymbolEntry entry : symbols) {
            ...
            if (entry.isConstant()) {
                ...
                if (entry.getConstantTypeLabel().contains("STRING")) {
                    ...
                    // Simplificación: tratar floats como enteros (truncar decimal)
                    String value = entry.getValue();
                    try {
                ...
                // Variable - usar DW para todos los números (simplificación)
                if (entry.getTypeLabel().equals("String")) {
                    asm.append("    ").append(varName).append(" DB 100 DUP ('$'), '$'\n");
                } else {
                    // Int y Float se tratan igual (como enteros de 16 bits)
                    asm.append("    ").append(varName).append(" DW ?\n");
                }
            }
        }
    ...
    private void generateCode(StringBuilder asm, List<String> lines) {
        for (int i = 0; i < lines.size(); i++) {
            ...
            // Procesar según tipo de instrucción
            if (line.equals("ET")) {
                // Etiqueta de inicio de ciclo
                String label = getOrCreateLabel(i + 1);
            ...
            } else if (containsBranchInstruction(line)) {
                // Condición + salto
                ...
                asm.append("    JMP ").append(label).append("\n");
            } else if (line.contains("CMP")) {
                // Línea con CMP pero sin salto (puede ser parte de WHILE IN)
                // Se procesa en conjunto con la siguiente línea
                continue;
            }
        }
    }
    ...
    private void processAssignment(StringBuilder asm, String line) {
        ...
        // Obtener expresión (todo menos var y :=)
        int endIndex = line.lastIndexOf(var);
        String expr = line.substring(0, endIndex).trim();
        
        if (expr.isEmpty()) {
            return; // Asignación vacía
        }
        
        // Si es asignación de string literal, ignorar (simplificación)
        if (expr.startsWith("\"") && expr.endsWith("\"")) {
            asm.append("    ; Asignacion de string literal a ").append(var).append(" (omitida)\n");
            return;
        }
        
        // Evaluar expresión y guardar en variable
    ...
    private void processWrite(StringBuilder asm, String line) {
        ...
        if (expr.startsWith("\"")) {
            ...
            // Es una expresión compleja (tiene espacios = tiene operadores)
            String result = evaluatePostfixExpression(asm, expr);
            asm.append("    MOV AX, ").append(result).append("\n");
    ...
    private void processBranchInstruction(StringBuilder asm, String line) {
        // Formato: condición BRANCH target
        String[] tokens = line.split("\\s+");
        String branchInst = null;
        String target = null;
        
        // Buscar la instrucción de salto
        ...
        int branchIndex = line.indexOf(branchInst);
        String condition = line.substring(0, branchIndex).trim();
        ...
        for (String token : tokens) {
            if (isLogicalOperator(token)) {
                if (token.equals("AND")) {
                    ...
                    // AND lógico: ambos deben ser verdaderos (!=0)
                    ...
                    String op2 = stack.pop();
                    String op1 = stack.pop();
                    // OR lógico: al menos uno debe ser verdadero (!=0)
                    asm.append("    MOV AX, ").append(op1).append("\n");
                    asm.append("    CMP AX, 0\n");
                ...
                // Guardar resultado de comparación como 1 (true) o 0 (false)
    ...
    private String evaluatePostfixExpression(StringBuilder asm, String expr) {
        ...
        for (String token : tokens) {
            if (isArithmeticOperator(token)) {
                ...
                // Es un operando - convertir a nombre sanitizado si es necesario
    ...
    private String resolveOperand(String operand) {
        // Si es un número o string literal, retornar el nombre sanitizado
        // De lo contrario, retornar tal cual (es una variable)
    ...
    private void loadValue(StringBuilder asm, String register, String value) {
        ...
        if (value.equals("@aux") || value.equals("@aux2") || value.equals("@result")) {
            // Variable auxiliar
            asm.append("    MOV ").append(register).append(", ").append(value).append("\n");
        } else if (value.startsWith("\"") && value.endsWith("\"")) {
            // Es un string literal - usar nombre sanitizado
            ...
            // Número literal - usar nombre sanitizado (ya está en la tabla de símbolos)
            String varName = sanitizeName(value);
    ...
    private boolean isNumericLiteral(String value) {
        // Detectar si es un literal numérico (entero o float)
    ...
    private boolean containsBranchInstruction(String line) {
